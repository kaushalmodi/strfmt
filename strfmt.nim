      ...
      of pkFmt:
        ...
        if p.recursive:
          if autonumber < 0:
            ...
            arg.inc
          # call "parse" to on recursively generated format string
          var fmtexpr: PNimrodNode = nil
          for e in generatefmt(p.fmt, args, arg):
            var newexpr = if p.fmt == nil: e.val else: newCall(bindsym"format", e.val, e.fmt)
            if fmtexpr != nil and fmtexpr.kind != nnkNilLit:
              fmtexpr = infix(fmtexpr, "&", newexpr)
            else:
              fmtexpr = newexpr
          result.add((argexpr, newCall(bindsym"parse", fmtexpr)))
        else:
          let f = p.fmt.parse
